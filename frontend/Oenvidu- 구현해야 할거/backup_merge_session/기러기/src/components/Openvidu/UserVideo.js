...
export default class UserVideoComponent extends Component {
	...
	updateExpressions = (event) => {
        console.log(event)
        const Emotions = {angry:'화남', disgusted:'역함', fearful:'공포', happy:'기쁨', neutral:'평온', sad:'슬픔', surprised:'놀람'}
    	var items = Object.keys(event).map(
            (key) => {return [Emotions[key], event[key]]}
        );
                ...
                <div className={styles.cam}>
                    {this.props.streamManager !== undefined ? (
                        <div className={styles.openvidu}>
                            <OpenViduVideoComponent streamManager={this.props.streamManager} updateExpressions={this.updateExpressions}/>
                        </div>
                    ) : null}
                </div>
