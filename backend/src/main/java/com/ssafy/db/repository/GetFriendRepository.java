...
public interface GetFriendRepository extends JpaRepository<GetFriend, Integer> {
     ...
     */
    @Query(value = "SELECT * FROM get_friend WHERE user_id=? OR get_friend_to_user_id " +
            "AND get_friend_is_accept IS true", nativeQuery = true)
    List<GetFriend> findByUserIdOrGetFriendToUserIdAndGetFriendIsAccept(int userId, int getFriendToUserId);
     ...
     */
    List<GetFriend> findUserIdByGetFriendToUserIdAndGetFriendIsAcceptFalse(int getFriendToUserId);
}
